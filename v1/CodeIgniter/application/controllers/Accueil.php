		...
		//chargement de la view des matchs dont le code saisi et pseudo choisi
			 if ($this->form_validation->run() == FALSE){
					...
					$data['match_id'] = $this->db_model->get_match_id($code);
					 ...
					 // si le pseudo saisi est vide, afficher l'erro et rester sur la page connecter(page_acceuil)
						if($pseudo == null){

							$this->load->view('jrs_sans_pseudo');
							$this->load->view('page_accueil',$data);

	
						}
						else {	//si le pseudo saisi n'existe pas 
								if ($meme_pseudo == 0){
