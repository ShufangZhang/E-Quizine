...
class Db_model extends CI_Model {
	...
	{
			$query = $this->db->query("SELECT * FROM t_compte_cpt join t_profil_plf using(cpt_id);");
	...
	public function get_match_id($code){
			$query = $this->db->query("select qz_intitule, qz_etat, mth_etat,mth_code,mth_intitule,mth_date_debut,mth_date_fin
					from t_match_mth
					join t_quiz_qz USING(qz_id)
					join t_question_ques USING(qz_id)
					join t_reponse_rep USING(ques_id)
	...
	//récupérer tous les quiz  d'un fromateur
	public function all_quiz($pseudo){
		$query = $this->db->query("SELECT qz_intitule,qz_etat,cpt_pseudo
	...
	//recuprer tous les match d'un formateur
	public function all_match($pseudo){
		$query = $this->db->query("SELECT mth_id,mth_intitule,mth_etat,mth_code,mth_date_debut,mth_date_fin,qz_intitule,cpt_pseudo
									from t_match_mth
									join t_quiz_qz using(qz_id)	
									join t_compte_cpt using(cpt_id)
									where cpt_pseudo='".$pseudo."'
	...
	//activer un match
	public function activer_match($match_id){
		$query=$this->db->query("update t_match_mth
								set mth_etat = 'A' where mth_id=".$match_id."
		;");
		return ($query);
	}
	//désactiver un match
	public function desactiver_match($match_id){
		$query=$this->db->query("update t_match_mth
								set mth_etat = 'D' where mth_id=".$match_id."
		;");
		return ($query);
	}
	/*
	//supprimer des questions
	public function supprimer_question($match_id){
		
		$query=$this->db->query("delete from t_question_ques
									join t_quiz_qz using(qz_id)
									join t_match_mth using(qz_id)
								 where mth_id=".$match_id."
		;");
		
	//supprimer des reponse
	public function supprimer_reponse($match_id){
		
		$query=$this->db->query("delete from t_match_mth
								 where mth_id=".$match_id."
		;");
		*/
	//supprimer des joueurs
		//supprimer un match
	public function supprimer_joueur($match_id){
		
		$query=$this->db->query("delete from t_joueures_jrs
								 where mth_id=".$match_id."
		;");
		return ($query);
	}
	...
	//raz un match
	public function reset_match($match_id){
		$query=$this->db->query("update t_match_mth set mth_date_debut=now(),mth_date_fin=null
								 where mth_id=".$match_id."
		;");
		return ($query);
	}
	//new match
	/*public function new_match($quiz_id){
		$match_intitule=$this->input->post('intitule');
		$quiz_id=$this->input->post('id');
		$match_code=$this->input->post('code');
		$date_debut=$this->input->post('date');

		$query=$this->db->query("insert into  t_match_mth
			 
		VALUES ('".$match_code."','".$match_intitule."','D','".$quiz_id."',null,'".$date_debut."',null)
			 ;"	
		);
		return ($query);
	}*/
